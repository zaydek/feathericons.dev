...
function splitParts(str: string) {
	return str.split(/(?=[A-Z])|[^a-zA-Z0-9]+/).filter(v => v !== "")
}
...
export function SearchApp() {
	...
	const values = useMemo(() => {
		...
		const $$search = splitParts(search)
			.map(v => v.toLowerCase())
			.join("-")
		for (const name of Object.keys(manifest)) {
			const $$name = splitParts(name)
				.map(v => v.toLowerCase())
				.join("-")
		...
		<>
			<div className="flex justify-center p-32">
				<div className="flex-basis--2e3px flex flex-col gap-32">
					<input
						ref={inputRef}
						className="font--400_18px_/_normal_$sans bg-#eee mx-16 h-64 rounded-1e3 px-32"
						type="text"
						value={search}
						onKeyDown={e => {
							if (e.key === "Escape") {
						...
						autoFocus
					/>
					<div className="grid-cols-repeat(auto-fill,_minmax(60px,_1fr)) grid-auto-rows-60 grid">
						{debouncedValues
							//// .filter(([, matches]) => matches)
							.map(([name, matches], index) => (
								<button
									key={name}
									className="center effect-icon-hover flex"
									onPointerOver={e => {
										setHoverName(name)
									}}
									onPointerLeave={e => {
