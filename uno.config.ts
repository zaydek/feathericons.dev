...
function convCssVars(value: string) { return value.replace(/\$([a-zA-Z][a-zA-Z-0-9]*)/g, "var(--$1)") }
function convUnderscores(value: string) { return value.replace(/_/g, " ") }
...
function desugar(rawValue: string | undefined, { sign, px }: { sign?: string, px?: boolean } = {}) {
	...
	const value = convUnderscores(convCssVars(rawValue))
	if (value.startsWith("$")) {
		return value
	} else {
		if (px) {
